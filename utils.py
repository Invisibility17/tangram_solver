...
import operator, math, random, copy, sys, os.path, bisect, re, functools
...
def distance(tupa, tupb):
    ...
    return math.hypot((tupa[0] - tupb[0]), (tupa[1] - tupb[1]))

def distance2(tupa, tupb):
    "The square of the distance between two (x, y) points."
    return (tupa[0] - tupb[0])**2 + (tupa[1] - tupb[1])**2
...
class PriorityQueue(Queue):
    ...
    def __contains__(self, item):
        ...
        return some(lambda kx: x == item, self.A)
    def __getitem__(self, key):
